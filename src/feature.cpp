...
FeatureExtractor20ms::FeatureExtractor20ms(float period_s) : PERIOD_S(period_s) {}

void FeatureExtractor20ms::begin(uint32_t sampleDelayUs, size_t maxBuffer) {
  Ts_us = sampleDelayUs;
  ...
  long nCalc = lroundf(PERIOD_S * fs);   // wie Python: int(round(PERIOD_S*fs))
  if (nCalc < 8) nCalc = 8;
...
void FeatureExtractor20ms::buildHamming() {
...
void FeatureExtractor20ms::resetWindow() {
  idx = 0;
  sumSq = 0.0f;
  maxAbs = 0.0f;
}

void FeatureExtractor20ms::addSample(float current_A) {
  if (idx >= N) return;
...
bool FeatureExtractor20ms::windowReady() const {
  return idx >= N;
}

size_t FeatureExtractor20ms::nearestBinIndex(float f_hz) const {
...
float FeatureExtractor20ms::goertzelMagBin(const float* x, size_t N, size_t k) const {
  if (N == 0) return 0.0f;
  if (k == 0) {
...
bool FeatureExtractor20ms::computeFeatures(float features[FEATURE_COUNT]) {
